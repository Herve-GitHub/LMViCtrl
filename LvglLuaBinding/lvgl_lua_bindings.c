...
// lv.timer_create(callback, period_ms [, oneshot])
static int l_lv_timer_create(lua_State* L) {
    luaL_checktype(L, 1, LUA_TFUNCTION);
    uint32_t period = (uint32_t)luaL_checkinteger(L, 2);
    int oneshot = lua_toboolean(L, 3); // 支持可选的 oneshot 参数
    ...
    lua_pushvalue(L, 1);
    cb_data->func_ref = luaL_ref(L, LUA_REGISTRYINDEX);
    cb_data->oneshot = oneshot; // 新增字段，标记是否 one-shot
