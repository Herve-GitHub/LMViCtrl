...
void getGatewayTags(const char* gateway_ip)
{
    ...
    // ==============================================
    char url[64] = { 0 };
    sprintf(url, "http://%s", gateway_ip);  // 拼接 http://xxx.xxx.xxx.xxx

    mg_connection* conn = mg_http_connect(&mgr, url, http_handler, NULL);
    if (!conn) {
        printf("连接失败：%s\n", gateway_ip);
        mg_mgr_free(&mgr);
        return;
    ...
    printf("成功从 %s 获取标签列表，已保存到 tag_list.txt\n", gateway_ip);
}