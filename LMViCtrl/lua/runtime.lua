...
local DateManage = require("common.DateManger")
...
-- 底层消息 → 交给 DateManage 自动刷新
-- 【全局唯一、永远不被覆盖的回调】
if not _G._lv_data_callback_registered then
    _G._lv_data_callback_registered = true

    lv.set_callbacks(function(connected)
        -- 连接状态
    end,
    function(id, val, status)
        -- 所有数据 100% 走这里，不被覆盖
        DateManage.update_value(id, val, status)
    end)
end
...
local function execute_event_action(action, self, e)
    ...
    if action_type == "freemaster" then
      ...
      if action_type == "datewrite" then
    local target = action.targetName and M.widgets[action.targetName]
    local params = action.params or {}
    local datapoint = params.datapoint
    local prop_val  = params.value

    -- 必须判断值是否为空！！！
    if not datapoint or datapoint == "" or not prop_val or prop_val == "" then
        print("[write] 数据点或值为空！！！")
        return
    end

    -- 调用底层写入（完全正确）
    local ok, ret = lv.write(datapoint, prop_val)
    print("[write] 发送结果：", ok)
...
--[[if action_type == "valuechange" then
    local target = action.targetName and M.widgets[action.targetName]
    local params = action.params or {}
    
    -- 取值
    local prop_name = tostring(params.property or "")
    local prop_val  = params.value
  ...
  --  local datapoint = params.datapoint
   -- lvgl.read(datapoint)
    ...
    -- ==============================================
    -- 🔥 万能清洗：把 "文本 (label)" 变成 "label"
    -- ==============================================
    local realProp = string.match(prop_name, "%((.-)%)")  -- 抽括号里的内容
    if realProp and realProp ~= "" then
        prop_name = realProp  -- 替换成真实属性名
    end

    -- 执行（和 set_property 完全一样）
    if target and type(target.set_property) == "function" then
        target:set_property(prop_name, prop_val)
        log_info("[valuechange] 最终执行: %s -> %s = %s",
            action.targetName, prop_name, tostring(prop_val))
    else
        log_warn("[valuechange] 目标控件无效")
    end
    return
end]]--
...
--[[if action_type == "valuechange" then
    ...
    -- 清洗属性名
    local realProp = string.match(prop_name, "%((.-)%)")
    if realProp then prop_name = realProp end

    -- 没有数据点直接退出
    if not datapoint or datapoint == "" then return end
    ...
    -- 【唯一正确】等数据回来再设置！
    -- ======================================================================
    data_client.add_message_handler(function(id, val)
        if id == datapoint then
            if target and target.set_property then
                target:set_property(prop_name, tostring(val))
                print("[✅ 成功] 设置控件值：", val)
            end
        end
    end)
...
--[[if action_type == "valuechange" then
    ...
    -- 2. 监听服务器推送的真实消息（自动更新，不用写死数据）
    lvgl.set_callbacks(nil, function(id, val, status)
        -- 只处理我们要的数据点
        if id == datapoint and target then
            target:set_property("label", val)
            print("✅ 真实服务器数据更新：", val)
        end
    end, nil)
...
--[[if action_type == "valuechange" then
    ...
    if not target or not datapoint then
        return
    end

    -- ==========================================
    -- 【关键】不管事件触不触发，直接立刻执行一次！
    -- ==========================================
    lvgl.read(datapoint)
...
if action_type == "valuechange" then
    ...
    local datapoint = action.params.datapoint

    if not targetName or not datapoint then return end

    -- ✅ 只绑定，不重复、不冲突
    DateManage.bind(datapoint, targetName, "label")

    -- ✅ 只订阅一次
  --  DateManage.read(datapoint)
...
function M.run(project)
    ...
    _G.PageManager.goto_page(1)
     ...
     -- 强制手动执行所有控件绑定在 value_changed 上的 valuechange 动作
  --[[  local function run_all_valuechange_actions()
        for widget_name, widget_inst in pairs(M.widgets) do
            -- 从工程原始屏幕数据找实例配置
            for _, scr in ipairs(screens) do
                for _, winst in ipairs(scr.widgets or {}) do
                    if winst.name == widget_name and winst.events then
                        -- 遍历 value_changed 下所有动作
                        local acts = winst.events["value_changed"]
                        if type(acts) == "table" then
                            for _, action in ipairs(acts) do
                                if action.type == "valuechange" and action.enabled ~= false then
                                    execute_event_action(action, widget_inst, nil)
                                end
                            end
                        end
                    end
    ...
    lv.timer_create(function()
        run_all_valuechange_actions()
        return false
    end, 100, nil)]]--


local function auto_subscribe_all_datapoints()
    local seen = {}
    local batch = {}
    for _, scr in ipairs(project.screens or {}) do
        for _, w in ipairs(scr.widgets or {}) do
            local events = w.events or {}
            for _, actions in pairs(events) do
                for _, action in ipairs(actions) do
                    if action.type == "valuechange" then
                        local dp = action.params and action.params.datapoint
                        local targetName = action.targetName
                        if dp and targetName and not seen[dp] then
                            seen[dp] = true
                            DateManage.bind(dp, targetName, "label")
                            table.insert(batch, dp) -- 👈 收集数据点
                            print("[订阅绑定] 数据点：", dp, " 控件：", targetName)
                        end
                    end
                end
            end
    ...
    -- 👇 必须加这一句：首次读取，触发后端推送
    if #batch > 0 then
        DateManage.read(table.unpack(batch))
    end
end

local ok = lv.timer_create(function()
    auto_subscribe_all_datapoints()
    return false
end, 500,true)
--lvgl.timer_delete(ok)
